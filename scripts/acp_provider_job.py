...
def main() -> int:
    ...
    if resume:
        ...
        rc, txs["createJob"] = ch.send(ch.core.functions.createJob(
    ...
    if (not resume) or ch.core.functions.getJob(jid).call()[1] == 1:
        ...
        if not resume:
            _rc, txs["setPayoutReceiver"] = ch.send(ch.core.functions.setPayoutReceiver(jid, agent),
                                                   "provider", "setPayoutReceiver(agent wallet)")
            _rc, txs["setBudget"] = ch.send(ch.core.functions.setBudget(jid, ch.usdc.address, amount, b""),
                                          "provider", f"setBudget {amount / 1e6:.2f} USDC")
            _rc, txs["fund"] = ch.send(ch.core.functions.fund(jid, ch.usdc.address, amount, b""),
                                     "client", f"fund {amount / 1e6:.2f} USDC")
        _rc, txs["setQualification"] = ch.send(ch.reg.functions.setQualification(ch.addr["provider"], 1),
                                             "deployer", "setQualification(operator, QUALIFIED)")
    ...
    _rc, txs["submit"] = ch.send(ch.core.functions.submit(jid, keccak(deliverable), b""), "provider",
                               "submit deliverable")
    ...
    _rc, txs["commitEvidence"] = ch.send(ch.reg.functions.commit(K._ev(eid, jid, 1, keccak(deliverable), FRESHNESS_BOUND,
    ...
    _rc, txs["approve"] = ch.send(ch.reg.functions.approve(jid, eid, keccak(text=f"evaluation-{jid}")),
                                 "deployer", "approve")
    ...
    _rc, txs["revokeQualification"] = ch.send(ch.reg.functions.setQualification(ch.addr["provider"], 3),
                                             "deployer", "revoke qualification (the refusal beat)")
    zero = keccak(text="acp-release")
    core_abi = K.decoding_abi()
    gate_errors = K.gate_error_abis()

    def release_body(simulate: bool) -> dict:
        """The release call, with the gate's errors attached for the decoder.

        Without `errorAbis` a refusal raised inside the hook reaches the caller as
        hex: the ABI that encodes the call is the core's, and the gate is not the
        core. The field is decoding-only, so it cannot change what is called.
        """
        body = {"contractAddress": ch.dep["core"], "network": K.NETWORK, "abi": core_abi,
                # attached, and ignored by the hosted API as of the probe above; it is here
                # so this lane needs no change the day the field is honoured
                "errorAbis": gate_errors, "functionName": "complete",
                "functionArgs": json.dumps([str(jid), "0x" + zero.hex(), "0x"])}
        if simulate:
            body["simulate"] = True
        return body

    # The revocation just landed and a simulate can be answered from a node whose view
    # has not caught up, which reads as a release that is fine when it is not. That is
    # the dangerous direction, so the refusal beat is polled rather than sampled once,
    # and every attempt is recorded.
    hold_attempts, st, sim, decoded = [], None, {}, None
    for attempt in range(1, 7):
        st, sim = K.kh(env, "POST", "/api/execute/contract-call", release_body(True),
                       idem=f"duality-acp-hold-{jid}-{attempt}")
        decoded = decode_release_blocked(str(sim.get("revertReason")))
        hold_attempts.append({"attempt": attempt, "httpStatus": st,
                              "wouldRevert": sim.get("wouldRevert"), "decoded": bool(decoded)})
        print(f"   attempt {attempt}: HTTP {st}  wouldRevert={sim.get('wouldRevert')}"
              f"  reason={str(sim.get('revertReason'))[:70]}")
        if sim.get("wouldRevert"):
            break
        time.sleep(5)
    decoded = decode_release_blocked(str(sim.get("revertReason"))) or decoded
    if decoded:
        print(f"   decoded by us: {decoded['error']} jobId {decoded['jobId']} reason {decoded['reason']}")
    keeper_hub_decoded = None
    if not decoded and "ReleaseBlocked" in str(sim.get("revertReason")):
        keeper_hub_decoded = str(sim.get("revertReason"))
    proof["keeperHubHoldSimulation"] = {"attempts": hold_attempts, "httpStatus": st,
                                        "wouldRevert": sim.get("wouldRevert"),
                                        "revertReason": sim.get("revertReason"), "signer": sim.get("from"),
                                        "decodedByUs": decoded,
                                        "decodedByKeeperHub": keeper_hub_decoded,
                                        "errorAbisAttached": len(gate_errors),
                                        "decodingNote": "the gate's own errors are attached so this "
                                                        "refusal decodes natively once the hosted API "
                                                        "ships the merged errorAbis field; as of this "
                                                        "run it is accepted and ignored, which "
                                                        "scripts/errorabis_probe.py measures and "
                                                        "artifacts/errorabis-probe.json records"}
    ...
    _rc, txs["reinstateQualification"] = ch.send(ch.reg.functions.setQualification(ch.addr["provider"], 1),
                                                "deployer", "reinstate qualification")
    ...
    converged = None
    for attempt in range(1, 7):
        st2, sim_ok = K.kh(env, "POST", "/api/execute/contract-call", release_body(True),
                           idem=f"duality-acp-ok-{jid}-{attempt}")
    ...
    st3, sent = K.kh(env, "POST", "/api/execute/contract-call", release_body(False),
    ...
    hold = proof["keeperHubHoldSimulation"]
    # The refusal must be OBSERVED and NAMED. Which side names it depends on the shape
    # the hosted API honours: once the caller can hand over the gate's errors, KeeperHub
    # names it and there is no raw hex left for this script to decode. Requiring the
    # local decode would fail a run that is strictly better than the one before it.
    refusal_observed = hold["wouldRevert"] is True
    refusal_named = bool(hold.get("decodedByUs")) or bool(hold.get("decodedByKeeperHub"))
    ok = (refusal_observed and refusal_named
          and readback["payoutReceiver"].lower() == agent.lower()
          and abs(after - before - amount / 1e6) < 1e-9)
    named_by = "KeeperHub" if hold.get("decodedByKeeperHub") else "this record"
    print("RESULT:", "PASS" if ok else "REVIEW",
          f"- held and named by {named_by}, then paid the agent's own wallet")
